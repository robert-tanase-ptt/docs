...
The AI-enhanced authentication feature improves the reliability of automatic login form detection in the [Website Scanner](/tools/website-scanner). By automatically identifying page elements, AI helps complete logins on complex or dynamic web pages where traditional detection methods struggle.
...
AI-enhanced authentication is designed to improve login reliability on complex pages where selector-based detection fails.

It is a fallback inside the **Automatic Login Form** flow, so you do not need to enable a separate setting.
