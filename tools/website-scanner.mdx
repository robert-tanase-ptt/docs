...
The Website Scanner is a **vulnerability scanner**: it actively tests web applications for security weaknesses. Discovered technologies are added to your [Attack Surface](/capabilities/attack-surface). Identified vulnerabilities are reported as [findings](/core/findings/findings).
...
<Note>
By default, the scanner follows redirects within the same domain (e.g., `example.com` to `www.example.com`). Configure this with [Follow redirects](/core/scans/follow-redirects). To scan a specific directory, include the path in the URL (e.g., `https://example.com/app/`).
</Note>
...
<Note>
For detailed authentication configuration, see [Authenticated scanning](/core/scans/authenticated-scanning#website-scanner).
</Note>
