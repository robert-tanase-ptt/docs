...
<Steps>
  ...
  <Step title="Filter responses">
    In Auto mode, discards 404 pages and detects soft 404s using an in-house heuristic detector. An [AI classifier](/ai/soft404detector) then runs a second pass on any response the heuristic marks as valid, to reduce false positives. In Manual mode, applies custom match/ignore conditions.
  </Step>
