...
The API Scanner tests REST and GraphQL APIs for security vulnerabilities. It understands API specifications (OpenAPI/Swagger, Postman collections) and uses GraphQL introspection to perform comprehensive security testing of your API endpoints.
...
The **Specification** toggle controls whether you provide an API specification file. When enabled, you can provide a specification to ensure comprehensive endpoint coverage.
...
The API Scanner follows a systematic approach:
...
- **Data breaches**: APIs often expose sensitive data and are a primary attack vector
- **OWASP API Top 10**: API-specific vulnerabilities require specialized testing
- **Business logic flaws**: APIs can expose internal business logic to attackers
- **Authentication bypass**: Weak API authentication can lead to unauthorized access
