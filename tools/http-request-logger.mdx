...
HTTP Request Logger is an **exploit helper tool**: it receives callbacks triggered by vulnerable applications to prove exploitation. It does not add data to your [Attack Surface](/using/attack-surface/attack-surface-overview) and does not generate [findings](/using/findings/findings). Captured requests are displayed in a custom report.
